    ...
    def update(self, new_time: dt.datetime):
        ...
        try:
            ...
            self.client.upload_file(
                self.temp_bookmark_path, self.bucket, self.s3_bookmark_path
            )
    ...
    def load(self):
        try:
            ...
            self.client.download_file(
                self.bucket, self.s3_bookmark_path, self.temp_bookmark_path
            )
            with open(self.temp_bookmark_path) as f:
                data = json.load(f)
                self.last_processed = dt.datetime.fromisoformat(data["last_processed"])
        except ClientError as e:
            if e.response["Error"]["Code"] == "404":
                print(
                    f"No existing bookmark found for {self.hydrophone}. Starting fresh."
                )
                self.last_processed = None


def process_upload_psd(
    start_time: dt.datetime,
    end_time: dt.datetime,
    hydrophone: Hydrophone,
    bookmark: Bookmark,
    file_connector: S3FileConnector,
):
    """
    ...
    psd_path, bb_path = pipeline.generate_parquet_file(
        start_time, end_time, upload_to_s3=True, partitioning=True
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process hydrophone audio data and generate PSD parquet files."
    )
    parser.add_argument(
        "--hydrophone",
        type=str,
